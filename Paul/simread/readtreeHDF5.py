...
class merger_tree:
    def __init__(self, basedir, skipfac, snapnum, filenum = 0, tree_start = -1, tree_num = -1, keysel = None):
        ...
        self.NhalosPerFile = hdf5lib.GetData(f, "Header/NhalosPerFile")[:]
...
#        self.trees = np.empty(tree_num - tree_start, dtype='object')
    ...
    def getMassesOfFirstProgenitors(self, ntree, nhalo):
        prog_list = []
        mass_list = []
        next = nhalo
        while next >= 0:
            prog_list.append(next)
            mass_list.append( self.trees[ntree]["Group_M_Mean200"][next] )
            next = self.trees[ntree]["FirstProgenitor"][next]
        return prog_list, mass_list
