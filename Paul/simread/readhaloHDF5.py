...
class HaloReader(object):
    def __init__(self, base, num, long_ids=False, snapbase='snap',
                 double_output=False, verbose=False, run=None, filename=None, **kwargs ):

        # a typical file path is base+snapbase+str(num).zfill(3) 
        if filename is not None:
            self.base, self.num = naming.return_snap_dir_and_num( filename )
            self.snapbase = naming.return_snapbase( filename )
        else:
            self.base = base
            self.num = num
        ...
        self.cat = readsubfHDF5.subfind_catalog(self.base, self.num, long_ids=long_ids,
...
#        print(self.file_num, len(self.filenames) )
