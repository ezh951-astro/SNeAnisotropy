...
for file_index,this_file in enumerate(files):
    ...
    gas_pos  = rs.read_block( this_file, 'POS ', parttype=4)
    gas_mass = rs.read_block( this_file, 'MASS', parttype=4)
    gas_vel  = rs.read_block( this_file, 'VEL ', parttype=4)
    ...
    gas_hsml = calc_hsml.get_particle_hsml( gas_pos[:,0], gas_pos[:,1], gas_pos[:,2], DesNgb=32  )
    ...
    massmap,face_image = makepic.contour_makepic( gas_pos[:,0], gas_pos[:,1], gas_pos[:,2], gas_hsml, gas_mass ,
          ...
          set_maxden = 1.0e-2, ## (gadget units, 10^10 msun/kpc^2 = 10^4 msun/pc^2)
          set_dynrng = 1.0e4  )
...
#a                      'cubehelix', 'rainbow' ]:
    ...
    for colormap in ['plasma']: 

       # total width = left_text_buffer + n_files 
       # total height = top_text_buffer + 1.0 + edge_aspect_ratio

       # left_buffer = left_text_buffer / (left_text_buffer + n_files)
       # image_width = 1.0 /  (left_text_buffer + n_files)
      ...
      im_ax.imshow( face_image, cmap=colormap, extent=(-boxsize/40.0, boxsize/40.0, -boxsize/40.0, boxsize/40.0) )
      ...
      im_ax.imshow( edge_image, cmap=colormap, extent=(-boxsize/40.0, boxsize/40.0, -boxsize/40.0*edge_aspect_ratio, boxsize/40.0*edge_aspect_ratio) )
...
all_im_fig.savefig('./plots/all_star_images.png', dpi=n_pixels-1)
plt.show()
