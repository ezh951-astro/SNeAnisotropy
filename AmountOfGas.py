...
for j,(RunName, SnapDir) in enumerate(zip(hd.runlabels,hd.snap_dirs)):
    ...
    if CALCULATE and (j in ActiveRunIDs):
        ...
        sfgasvel_R = np.where( np.isnan(sfgasvel_R), np.zeros_like(sfgasvel_R), sfgasvel_R )
        sfgasvel_Z = np.where( np.isnan(sfgasvel_Z), np.zeros_like(sfgasvel_Z), sfgasvel_Z )
...
axs.plot( dat_z01[:,0], dat_z01[:,6]/1.e7, label='In-Disk Fav.', alpha=0.6 )
axs.plot( dat_z1[:,0], dat_z1[:,6]/1.e7, label='Isotropic', alpha=0.6 )
axs.plot( dat_z10[:,0], dat_z10[:,6]/1.e7, label='Out-Disk Fav.', alpha=0.6 )

axs.set_ylabel('Mass of Star-Forming\nGas ' + r'[$10^7$ ${\rm M}_{\odot}$]', fontsize=16)

axs.tick_params( labelsize=15, direction='in' )
...
'''
axs[0].plot( dat_z01[:,0], dat_z01[:,4], color='C0', alpha=0.69, linewidth=1 )
axs[0].plot( dat_z01[:,0], dat_z01[:,5], color='C1', alpha=0.69, linewidth=1 )
ax0star = axs[0].twinx()
ax0star.plot( dat_z01[:,0], dat_z01[:,6]/1.e7, color='C3', alpha=0.69, linewidth=1 )
ax0star.set_ylim(( YMIN_PLOT/40., YMAX_PLOT/40. ))

#special_blue_1 = np.where( dat_z1[:,1] < 8.1, dat_z1[:,1], 8.15*np.ones_like(dat_z1[:,1]) )
axs[1].plot( dat_z1[:,0], dat_z1[:,4], color='C0', alpha=0.69, linewidth=1 )
axs[1].plot( dat_z1[:,0], dat_z1[:,5], color='C1', alpha=0.69, linewidth=1 )
ax1star = axs[1].twinx()
ax1star.plot( dat_z1[:,0], dat_z1[:,6]/1.e7, color='C3', alpha=0.69, linewidth=1 )
ax1star.set_ylim(( YMIN_PLOT/40., YMAX_PLOT/40. ))

axs[2].plot( dat_z10[:,0], dat_z10[:,4], color='C0', alpha=0.69, linewidth=1, label = r'Radial Velocity' )
axs[2].plot( dat_z10[:,0], dat_z10[:,5], color='C1', alpha=0.69, linewidth=1, label = r'Vertical Velocity' )
ax2star = axs[2].twinx()
ax2star.plot( dat_z10[:,0], dat_z10[:,6]/1.e7, color='C3', alpha=0.69, linewidth=1, label = 'SF Gas Mass' )
ax2star.set_ylim(( YMIN_PLOT/40., YMAX_PLOT/40. ))
...
ax1star.set_ylabel(r'Star-Forming Gas [$10^7$ ${\rm M}_{\odot}$]',fontsize=16.5)
axs[1].set_ylabel(r'Avg. Vel. of SF Gas [km/s]',fontsize=16.5)
